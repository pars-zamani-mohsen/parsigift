        ...
        @if(count($all))
            @foreach ($all as $key => $item)
                <tr>
                    ...
                    <td>{{ $item['url'] }}</td>
                    <td>{{ $item['mobile'] }}</td>
                    <td><a href="{{ url('/'. App\Http\Controllers\HomeController::fetch_manager_pre_url() . '/gift/' . $item['gift_id'] . '/edit') }}">#{{ $item['gift_id'] }}-{{ $item->gift->title ?? '' }}@if($item->gift->amount) (مبلغ: {{ $item->gift->amount ?? 0 }})@endif</a></td>
